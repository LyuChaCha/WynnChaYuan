...
enemies when transferring {~}+ Mana.`
- 舊譯：當 Arcane Transfer 轉換 {~} 點以上魔力時，
會對周圍敵人造成傷害。
- 新譯：祕法回流提取超過 {~} 魔力時，對附近的敵方造成傷害。

## Sunshower

- 原文：`Heal emits a strong light,
damaging nearby enemies.`
- 舊譯：Heal 會散發強光，
對周圍敵人造成傷害。
- 新譯：治癒發出一道強光，對附近的敵方造成傷害。

## Astral Fragmentation

- 原文：`Meteor scatters into debris upon
landing, dealing additional damage.`
- 舊譯：Meteor 落地後會碎裂成碎片，
造成額外傷害。
- 新譯：隕石落地時會有碎片濺出，造成額外傷害。

## Astral Fragmentation（數值對不上，未自動套用）
...
ethereal image of yourself at your location.`
- 舊譯：蹲下時施放 Teleport,會在你所在的位置
留下一個自己的虛影。
- 新譯：蹲下使用傳送將會在原地留下一個虛影。
再度使用此技能時，傳送至虛影的位置。
...
tornado that deals damage every {~}s.`
- 舊譯：施放 Ice Snake 時改為召喚一道冰凍龍捲風，
每 {~}s 造成一次傷害。
- 新譯：使用冰凍龍捲風取代你的冰蛇，每 {~} 秒造成一次傷害。

## Frozen Tornado（數值對不上，未自動套用）
...
tornados or snakes at the same time.)`
...
- 原文：`Ice Snake will summon {~} snakes.`
- 舊譯：Ice Snake 會召喚 {~} 條蛇。
- 新譯：冰蛇會召喚 {~} 條冰蛇。
...
## Arcane Restoration（數值對不上，未自動套用）
...
- 原文：`Double Meteor's speed and increase its damage.`
- 舊譯：使 Meteor 的速度加倍，並提升其傷害。
- 新譯：增加隕石的傷害並使其速度翻倍。

## Void Acceleration

- 原文：`Meteor Shower summons meteors +{~} faster.`
- 舊譯：Meteor Shower 召喚隕石的速度加快 +{~}.
- 新譯：流星雨召喚隕石的速度增加 {~}。
...
- 原文：`Frozen Tornado drifts towards nearby enemies.`
- 舊譯：Frozen Tornado 會朝附近的敵人漂移。
- 新譯：冰凍龍捲風會朝著附近的敵人移動。

## Sentient Snake

- 原文：`Ice Snake will follow the direction you're
facing, allowing you to control it.`
- 舊譯：Ice Snake 會跟著你面向的方向前進，
讓你能夠操控它。
- 新譯：冰蛇會朝著你面對的方向移動。

## Arcane Power

- 原文：`Arcane Restoration will restore mana {~}s faster.`
- 舊譯：Arcane Restoration 回復魔力的速度加快 {~}s.
- 新譯：祕法再生的效果加快 {~} 秒。

## Arcane Power（數值對不上，未自動套用）
...
created, damaging enemies inside it.`
- 舊譯：Dimensional Tear 生成時會劇烈爆發，
對裡面的敵人造成傷害。
- 新譯：次元裂隙在生成時會發生爆炸，對敵方造成傷害。

## Rift Rupture

- 原文：`While Riftbound, Etheric Slash implodes your
bound rift instead, damaging nearby enemies.`
- 舊譯：處於 Riftbound 狀態時，Etheric Slash 改為引爆
你綁定的裂隙，對周圍敵人造成傷害。
- 新譯：當玩家 裂隙繫身狀態時，
裂界斬改為使你的裂隙坍縮，對敵方造成傷害。
...
will make your next Heal activate Sunflare.`
- 舊譯：在 {~2} 秒內治療了你最大生命的 {~1},
下一次 Heal 就會啟動 Sunflare.
- 新譯：{~}秒內治癒量超過你最大生命值的 {~} 後，
你的下一個治癒會發動日耀。
日耀發動期間，你會回復附近友方的生命和魔力，

## Sunflare（數值對不上，未自動套用）
...
Time Dilation from decaying for {~}s.`
- 舊譯：用普攻命中敵人，可讓 Time Dilation
停止衰減 {~}s.
- 新譯：時流偏移在提供移動速度的同時增加你的普通攻擊傷害。
使用普通攻擊命中敵人能使移動速度不因停止衝刺而降低，持續 {~} 秒。
...
- 原文：`Heal and Arcane Transfer weaken nearby enemies.`
- 舊譯：Heal 與 Arcane Transfer 會使周圍敵人虛弱。
- 新譯：治癒和祕法回流會弱化附近的敵人。

## Devitalize（數值對不上，未自動套用）
...
Dimensional Tear, making you Riftbound.`
- 舊譯：蹲下時施放 Heal,會把你綁定到你的
Dimensional Tear 上，使你進入 Riftbound 狀態。
- 新譯：蹲下並施放治癒使你和你的次元裂隙綁定，進入裂隙繫身狀態。

## Riftbound（數值對不上，未自動套用）
...
{~2} 點魔力，每 {~3} 秒一次，直到
魔力耗盡為止。
- 新譯：蹲下並施放祕法回流會召喚出一顆電球，
每 {~} 秒對附近敵人造成一次傷害並消耗 {~} 點魔力。
電球的傷害和魔力消耗隨著每一次命中提高 {~}。

## Diffraction（數值對不上，未自動套用）
...
by a Riftspawn while active.`
- 舊譯：Dimensional Tear 生效期間，
現在會有一隻 Riftspawn 棲居其中。
- 新譯：次元裂隙中將會有一個 裂隙之裔 躲在其中。

## Portal to the Beyond（數值對不上，未自動套用）
...
the tornado damages an enemy.`
- 舊譯：龍捲風每對敵人造成一次傷害，
就額外提升 Frozen Tornado
+{~} 的傷害。
- 新譯：冰凍龍捲風每次命中目標時增加 {~} 傷害。

## Blitz

- 原文：`Increase the damage of Ice Snake
...
- 原文：`Judrajim applies Unstable {#} to enemies.`
- 舊譯：Judrajim 會對敵人施加 Unstable {#} .
- 新譯：釋放毀滅之雷的魔法會使命中目標 失衡 {#}。

## Induced Instability

- 原文：`Meteor, Pyrokinesis and Powder Specials
consume Unstable {#} to deal +{~} damage.`
- 舊譯：Meteor、Pyrokinesis 與粉末特效會
消耗 Unstable {#} 以造成 +{~} 傷害。
- 新譯：隕石、馭火和粉末技能會消耗 失衡 {#} 並增加 {~} 傷害。
...
## Gravitational Collapse（數值對不上，未自動套用）
