...
> [!IMPORTANT]
> **目前是 Beta（0.1.9_3）。** 功能與譯文都還在調整，遇到問題請到 Discord 找
> **LyuChaCha**，任何回報都歡迎。
>
...
| **更新提示** | 有新版時在聊天室說一次，附 GitHub 下載連結；F6 →「更新說明」看得到本版改了什麼 |
...
| 語言 | 狀態 |
|---|---|
| `zh_tw` 繁體中文 | 主要語言，其他語言以它的進度為基準 |
| `zh_cn` 簡體中文 | 介面、技能樹、任務對話都有 |
| `ja_jp` 日文 | 介面、技能樹、名牌、物品敘述與任務對話都有 |
| `ru_ru` 俄文 | 介面、技能樹、名牌與物品敘述；任務對話翻譯中 |
| `ko_kr` 韓文 | 只有少量介面 |

在 **F6 →「資料」** 切換，不必改遊戲本身的語言，也不用重開遊戲：

- **譯文語言**：要看哪一種語言的翻譯。
- **輔助語言**：譯文語言還沒翻到的句子，改顯示這一種。
- **介面語言**：F6 設定畫面本身用的語言。

一條都還沒翻的語言不會打包進 jar，也不會出現在清單裡。
每一種語言還缺哪些檔案見 [docs/PROGRESS.md](docs/PROGRESS.md)。
...
模組啟動時會讀它，跟手上這個 jar 不一樣就在聊天室說一次，附 GitHub 的
...
- 對話框裡的中日韓與俄文字形使用 [Fusion Pixel 10px](https://github.com/TakWolf/fusion-pixel-font)
  比例模式（SIL OFL 1.1，授權全文見 `assets/wynnchayuan/font/ofl-fusion.txt`）。
  它的大寫高度正好 7 像素，跟 Wynncraft 對話框的英文同高。
  字型**按語言分**：同一個碼位在不同地區的寫法不一樣，目前附繁體中文（`zh_tw`）、
  簡體中文（`zh_cn`）、日文（`ja_jp`）、韓文（`ko_kr`）與俄文（`ru_ru`）。
  點陣字沒有哪一套是全的，所以譯文只要有一個字
