...
有些詞條同時有「實數」與「百分比」兩種，英文的標籤<b>一模一樣</b>：

```
Health Regen    -605      ← 實數
Health Regen    -162%     ← 百分比
```

中文常常要分開講。**在鍵的尾端加一個 `%`**，那條就只會用在數值是百分比的時候：

```json
"Health Regen":  "生命回復",
"Health Regen%": "生命回復百分比"
```

沒有加 `%` 版本的標籤完全不受影響，照舊用同一條。

**只在真的兩種都有的時候加。** 近戰反傷、遠程反傷本來就只有百分比，
魔力回復、生命竊取、中毒本來就只有實數——替它們多開一個 `%` 版本，
只會讓後面的人以為遊戲裡真的有兩種，反而翻亂。目前有 `%` 版本的是
生命回復與各種傷害；其他的請先在遊戲裡確認看得到兩種形態再加。
