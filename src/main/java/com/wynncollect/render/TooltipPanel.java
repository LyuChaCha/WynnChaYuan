...
public final class TooltipPanel {
    ...
    public static void render(GuiGraphics graphics, List<Component> tooltip,
        ...
        if (!tooltip.equals(lastDiagnosed)) {
            ...
            WynnCollect.store().noteEvent(lines.isEmpty() ? "panel.noMatch" : "panel.shown");
            if (lines.isEmpty()) {
        ...
        int gap = WynnCollect.config().panelGap();
        int panelW = width(mc, lines) + 8;      // 8 = tooltip 左右內距
        int panelH = lines.size() * 10 + 8;

        int x;
        int y;
        if (WynnCollect.config().panelAnchor() == CollectorConfig.PanelAnchor.FIXED) {
            x = WynnCollect.config().fixedX();
            y = WynnCollect.config().fixedY();
        } else {
            ...
            x = switch (WynnCollect.config().panelSide()) {
