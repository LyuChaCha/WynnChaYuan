...
public final class TrackerListener {
    ...
    public void onTrackerUpdate(ActivityTrackerUpdatedEvent event) {
        WynnCollect.store().noteEvent("tracker.updated");
        ...
        if (task != null && !GlyphSplitter.isGlyphOnly(task)) {
            ...
            if (PlayerDataFilter.carriesPlayerData(template)) {
                WynnCollect.store().noteEvent("tracker.blocked.playerData");
            } else {
                WynnCollect.store().record(template, "desc", "quest", "tracker/task");
            }
        }

        TrackerOverlay.setCurrent(name, task, WynnCollect.translations());
    }
}
