...
import com.wynncollect.capture.DialogueBuffer;
import com.wynncollect.capture.GlyphSplitter;
import com.wynncollect.capture.PlayerDataFilter;
...
public final class CaptureListener {
    ...
    public void onDialogueStarted(NpcDialogueEvent.Started event) {
        WynnCollect.store().noteEvent("dialogue.Started");
        feed(event);
    }

    @SubscribeEvent(priority = EventPriority.LOWEST)
    public void onDialogueUpdated(NpcDialogueEvent.Updated event) {
        WynnCollect.store().noteEvent("dialogue.Updated");
        feed(event);
    }

    @SubscribeEvent(priority = EventPriority.LOWEST)
    public void onDialogueFinished(NpcDialogueEvent.Finished event) {
        WynnCollect.store().noteEvent("dialogue.Finished");
        feed(event);
    }
    ...
    public void onDialogueEnded(NpcDialogueEvent.Ended event) {
        WynnCollect.store().noteEvent("dialogue.Ended");
        feed(event);
    }
    ...
    private void feed(NpcDialogueEvent event) {
        ...
        DialogueOverlay.setCurrent(styled, WynnCollect.translations());
    ...
    private void record(String completed) {
        if (completed == null || !WynnCollect.config().collect()) {
            return;
        }
        WynnCollect.store().record(
    ...
    public void onChat(ChatMessageEvent.Match event) {
        WynnCollect.store().noteEvent("chat." + event.getRecipientType());
        if (!WynnCollect.config().collect()) {
            return;
        }
        ...
        if (PlayerDataFilter.carriesPlayerData(template)) {
            WynnCollect.store().noteEvent("chat.blocked.playerData");
    ...
    public void onLabel(TextDisplayChangedEvent.Text event) {
        ...
        if (full != null && !GlyphSplitter.isGlyphOnly(full)) {
            ...
            if (!template.isBlank() && GlyphSplitter.hasLetter(template)) {
                WynnCollect.store().record(template, "name", "npc", "npc/nametag");
            }
        }
     ...
     *
     * <p>風險由三件事壓住：{@link com.wynncollect.translate.LineTranslator} 保證符號連同
     * 原字型整段填回、地名原樣保留、佔位符對不上就整行放棄。所以最壞情況是「沒翻到」，
     * 不會變成亂碼。
     */
    private void translateNametag(TextDisplayChangedEvent.Text event) {
        CollectorConfig.NametagMode mode = WynnCollect.config().nametagMode();
        if (!WynnCollect.config().showPanel() || mode == CollectorConfig.NametagMode.OFF) {
            return;
        }
        try {
            ...
            Component translated =
                    LineTranslator.translate(original, WynnCollect.translations());
            if (translated != null) {
                event.setText(StyledText.fromComponent(translated));
            }
        } catch (Throwable t) {
            WynnCollect.store().noteEvent("render.nametagError");
        }
    }
}
