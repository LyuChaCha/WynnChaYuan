...
public final class ChatLogTest {
    ...
    public static void main(String[] args) {
        ...
        ChatLog.add("Hello", "哈囉");
        ChatLog.add("World", null);
        check("記了兩則", ChatLog.size() == 2);

        List<ChatLog.Entry> recent = ChatLog.recent();
        check("由新到舊", recent.get(0).original().equals("World"));
        check("舊的排後面", recent.get(1).original().equals("Hello"));

        // 空白的不收——分隔線、純排版符號那種複製了也沒用
        ChatLog.add("   ", "有譯文但原文是空的");
        ChatLog.add(null, "原文是 null");
        check("空白與 null 都不收", ChatLog.size() == 2);

        // 顯示模式：畫面上看到什麼，複製起來就是什麼
        ChatLog.Entry both = new ChatLog.Entry("Hello", "哈囉");
        check("只顯示原文就複製原文",
        ...
        ChatLog.Entry only = new ChatLog.Entry("Player says hi", null);
        check("沒譯文時就地取代仍給原文",
                only.forMode(true, false).equals("Player says hi"));
        check("沒譯文時兩份模式也只給原文",
                only.forMode(false, true).equals("Player says hi"));
        ChatLog.Entry blank = new ChatLog.Entry("Player says hi", "   ");
        check("譯文只有空白也當成沒有",
        ...
        for (int i = 0; i < ChatLog.CAPACITY + 50; i++) {
            ChatLog.add("line " + i, null);
        }
        check("不會超過容量", ChatLog.size() == ChatLog.CAPACITY);
        List<ChatLog.Entry> full = ChatLog.recent();
        check("留下的是最新的",
                full.get(0).original().equals("line " + (ChatLog.CAPACITY + 49)));
        check("丟掉的是最舊的",
                full.get(full.size() - 1).original().equals("line 50"));
