...
class Kind:
    ...
    def __init__(self, folder: str, bundle: str, domain: str, what: str):
    ...
    def note(self) -> str:
        return (f"{self.what}。這個檔案是<b>產生物</b>——請改 {self.folder}/ 底下"
                f"對應的檔案，再跑 tools/quest-bundle.py。直接改這裡下次會被蓋掉。")

    @property
    def part_note(self) -> str:
        return f"一個{'任務' if self.folder == 'quest' else '故事'}一個檔。改完跑 tools/quest-bundle.py。"


KINDS = (
    Kind("quest", "quest-dialogue.json", "quest-dialogue", "任務對話"),
    Kind("secret", "secret-dialogue.json", "secret-dialogue", "祕密發現的對話"),
)
